...
var getHitNumbers = function(body){
	var names = [];
	var $ = cheerio.load(body);
	var links = $('div.rb ol li .stat');
	for (var i = 0; i < links.length; i++) {
		names.push(links.eq(i).text().replace(',',''));
	};
	return names;
}
...
listPageURLs.push('http://www.soku.com/newtop/teleplay.html')  //tele
listPageURLs.push('http://www.soku.com/newtop/variety.html')	//variety
listPageURLs.push('http://www.soku.com/newtop/anime.html')	    //cartoon
listPageURLs.push('http://www.soku.com/newtop/movie.html')	    //movie
...
function oneListPageHandler(body,callback){
	...
	var allHitNumbers = getHitNumbers(body);
	var fileData = "";
	for (var i = 0; i < allVideoNames.length; i++) {
		...
		var titleWhere = '=  \''+ temp + '\'';
		if(temp.indexOf(' ') != -1){
			titleWhere = 'like '+ '\'%'+temp.split(' ')[0]+'%\' and title like ' + '\'%'+temp.split(' ')[1]+'%\'';
		}
		fileData += sqlData + titleWhere + ';\n' + "UPDATE gx_video SET hits = " + allHitNumbers[i] +' where title ' +titleWhere + ';\n';
	};
