var db = require('./db'),cheerio = require('cheerio'),
	...
	jsdom = require('jsdom'), U = require('./utility'), M = require('./mysqlHelper')

var getPlayList = U.getPlayList,getTimeStamp = U.getTimeStamp,getColId = U.getColId,getFirstLetter = U.getFirstLetter,
	getUpandDown = U.getUpandDown, checkTerminated = U.checkTerminated,
	getVideoModel = M.getVideoModel, insertVideo = M.insertVideo,perpareSQL = M.perpareSQL
...
function oneDetailPageSeries(detailPageURLs,_callback){
	...
	series['requestDetailPage'] = function(callback){
		request({uri: detailPageURL,timeout:300000}, function(err, response, body){
	        ...
	        if(!body){
		        ...
		        oneDetailPageHandler(body,function(error,_video){
		        	video = _video;
		        	callback(error);
		        });	
	        }
	...
	series['updateLinkStatus'] = function(callback){
		if(!video){
			...
			var isTerminated = checkTerminated(video);

			if(!isTerminated){
				newCount++;
				fs.appendFileSync('test/newVideo.txt',newCount + '. ' +video.title + ' | '+ video.year + '| '+ detailPageURL + "\n");
			}
	...
	async.series(series,function(error,result){
		...
		console.log('[INFO]:  -- * '+detailPageURL + '* --');
		if(video){
			count ++;
			console.log(count + '. '+video.title  + ' has been added.')
		}else{
