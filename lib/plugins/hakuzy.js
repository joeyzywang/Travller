...
var oneDetailPageHandler = function (body,callback){
	...
	video.intro = 		(bodyStr.between('<!--影片状态开始代码-->','<!--影片状态结束代码-->').s)
	...
	var temp = bodyStr.between('<!--影片更新时间开始代码-->','<!--影片更新时间结束代码-->').s;
	if(temp){
		var dateTemp = temp.substring(0,temp.indexOf(' '));
		var dateArray = dateTemp.split('-');
		video.addtime = getTimeStamp(dateArray[0],dateArray[1],dateArray[2]);		
	}else{
	...
	//video.score = 8.5 - Math.floor(Math.random()*3);
	video.score = 0;
	...
	video.cid = getColId(category,video.title,video.area,video.intro,subtitle,getPlayNumber(bodyStr));
	...
	if(video.cid == 999){
		...
		checkUpdateAndGetSQL(video,function(error,sql){
			if(error || sql == ""){ //If error or no sql,  no need to update
				...
				executeSql(sql,function(error,result){
					if(error){
...
var checkTerminated = function(video){
	//
	if(video.cid == 999) return true; //999 是QMV热片，不入库
...
var getColId = function(str,title,area,intro,subtitle,playNumber){
	...
	if((str=='综艺其它' && title.indexOf("比赛") != -1)||(str.indexOf("综艺其它") != -1 && title.indexOf("NBA") != -1)){
		...
		return 6;
	}else if ( str == "动漫片" && ( ( (intro=='0' || intro == 0 ) && playNumber < 5 ) ) ) { // 完结的动漫片，并且基数少于5，归于电影
		return 39;
	}else if(str == "国产剧" || str == "大陆剧"){
		return 15;
	}else if(str == "港台剧" && area == "台湾"){
		return 16;
	}else if(str == "港台剧" && area == "香港"){
		return 17;
	}else if(str == "日韩剧" && area == "韩国"){
		return 18;
	}else if(str == "日韩剧" && area == "日本"){
		return 19;
	}else if(str == "欧美剧" || str == "美剧"){
		return 20;
	}else if(str == "海外剧"){
		return 21;
	}else if(str == "欧美剧" && isBritsh(title)){ //英剧
		return 23;
	}else if(str == "港台剧" && area == "泰国"){
		return 24;
	}else if(str == "港台剧" && area == "其他"){ //印度剧
		return 25;
	}else if(str == "港台剧" && area == "其他"){ //新加坡剧
		return 26;
	}else if(str == "动漫"){ //动漫的父目录
		return 3;
	}else if(str == "动漫片" && area == '大陆'){
		return 27;
	}else if(str == "动漫片" && area == '日本'){
		return 28;
	}else if(str == "动漫片" && area == '欧美'){
		return 29;
	}else if(str == "综艺"){
		return 4;
	}else if(str == "综艺其它" && area =='大陆'){ 
		return 30;
	}else if(str == "综艺其它" && area == '台湾'){
		return 31;
	}else if(str == "综艺其它" && area == '香港'){
		return 32;
	}else if(str == "综艺其它" && area == '韩国'){
		return 33;
	}else if(str == "综艺其它" && area == '日本'){
		return 34;
	}else if(str == "综艺其它" && area == '欧美'){
		return 35;
	}else if(str == "音乐" && (title.indexOf('演唱会')!=-1)){
		return 36;
	}else if(str == "音乐" && (title.indexOf('晚会')!=-1)){
		return 37;
	}else if(str == "音乐"){ //其他娱乐
		return 38;
	}else if(str == "微电影"){
	...
	var video_exist = queryVideo('gx_video',[],{title:video_latest.title},function(error,videos){
		...
		if(videos.length > 0){
			...
			if(latest_serial_number > exist_serial_number){
				checkTerminated(video_latest);
				return callback(null,M.getUpdateSQL('gx_video',{qvod:video_latest.qvod,addtime:video_latest.addtime,intro:video_latest.intro},{title:video_latest.title})+ ";\n")
			}else{
				return callback(null,"");
			}
		}else{
			//没找到有同名的影片
			if(video_latest.intro != 0 && video_latest.intro != '0')
